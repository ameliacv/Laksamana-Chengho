...
public class AIFollow : MonoBehaviour {
	...
	float pickNextWaypointDistance = 1.5F;
	...
	void Awake(){
		...
		healtplayer = Player.GetComponent <PlayerHealth>();
	...
	public void ReachedEndOfPath () {
		...
		healtplayer.TakeDamage(attackDamage);
		...
		while ((currentWaypoint - tr.position).sqrMagnitude < pickNextWaypointDistance*pickNextWaypointDistance) {
			...
			if (pathIndex >= path.Length) {
				...
				if ((currentWaypoint - tr.position).sqrMagnitude < (pickNextWaypointDistance*targetReached)*(pickNextWaypointDistance*targetReached)) {
					...
					if(healtplayer.currentHealt <= 0){
						anim.Play("Idle_01");
					}
					/*			if (playerInRange && healtplayer.currentHealt >0) {
						...
						if (!playerInRange && healtplayer.currentHealt >0){
							anim.SetBool("Player_Out",true);
							Debug.Log("Player Not In Range, Enemy Jalan");
						}
						if(healtplayer.currentHealt <= 0){
